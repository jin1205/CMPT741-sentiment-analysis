%% CMPT-741 course project: sentiment analysis base on Convolutional Neural Network
% author: Jin Zhang, Fan Liu
...
W_out_list = cell(fold, 1);
B_out_list = cell(fold, 1);
auc_train_list = cell(fold, 1);
auc_val_list = cell(fold, 1);
...
for fold_index = 0: (fold-1)
    ...
    % separate data into training_data and validation_data
    training_data = [data(1 : fold_size*fold_index, :); data(fold_size*(fold_index+1)+1 : end, :)];
    ...
    for t = 1: numIterations
        ...
        for i = 1: length(training_data)        
            ...
            o = vl_nnconv(z_reshape, reshape(W_out, 6, 1, 1, 2), B_out);
            ...
            [dlossdz, dlossdW_out, dlossdB_out] = ...
                vl_nnconv(z_reshape, reshape(W_out, 6, 1, 1, 2), B_out,dlossdo);
    ...
    W_out_list{fold_index+1} = W_out;
    B_out_list{fold_index+1} = B_out;
    auc_train_list{fold_index+1} = auc_train;
    auc_val_list{fold_index+1} = auc_val; 
    
    fprintf('validation set = fold %i is done\n', fold_index+1);
    fprintf('********************** \(^o^)/ ********************** \n');
end
