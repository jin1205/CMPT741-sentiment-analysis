...
for t = 1: numIterations
    ...
    for i = 1: length(training_data)        
        ...
        o = vl_nnconv(z_reshape, reshape(W_out, length(filter_size) * n_filter, 1, 1, n_class), B_out);
        ...
        [dlossdz, dlossdW_out, dlossdB_out] = ...
            vl_nnconv(z_reshape, reshape(W_out, length(filter_size) * n_filter, 1, 1, n_class), B_out,dlossdo);
        ...
        W_out = W_out - rate .* reshape(dlossdW_out, length(filter_size) * n_filter, n_class);
    ...
    for i = 1: length(training_data)
        ...
        o = reshape(z_reshape * W_out, n_class, 1) + B_out;
        if o(1) > o(2)
            y = 1;
        else y = 0;
        end
